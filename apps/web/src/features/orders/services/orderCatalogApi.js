...
export const STOCK_ENFORCED_SUPPLIER_IDS = new Set(["8"]);

export function supplierEnforcesStock(supplierId) {
...
export async function fetchOrderStores() {
...
export async function fetchOrderProducts(supplierId) {
...
export async function fetchOrderInventory(supplierId) {
  ...
  for (const row of data) {
    if (row?.id != null) {
      map[String(row.id)] = Number.isFinite(row.stock) ? row.stock : 0;
    }
  }
  return map;
}

export function getOrderProductName(product, lang) {
  if (lang === "zh") {
...
export function getOrderProductUnit(product) {
  return product.unit || product.specification || "—";
}

export function getOrderProductVariants(product) {
