...
export type LiaoSwingHandlers = {
  isDragging: boolean;
  rotationVar: ReturnType<typeof useMotionTemplate>;
...
export function useLiaoSwing(): LiaoSwingHandlers {
  const [isDragging, setIsDragging] = useState(false);
  const timeoutIds = useRef<number[]>([]);
  const animationRef = useRef<MotionAnimationControls | null>(null);
  const pointerState = useRef<SwingPointerState>({
    pointerId: null,
    startX: 0,
    startRotation: 0,
    lastX: 0,
  ...
  function clampRotation(nextRotation: number): number {
    return Math.max(
      -maxHorizontalRotation,
  ...
  function settleSwing(targetRotation: number): void {
