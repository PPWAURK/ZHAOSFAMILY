...
export function TrainingAchievements({
  ...
  const averageQuizScore = useMemo(() => {
    const scoredRecords = completed.filter(
      (record) => record.bestQuizScore !== null,
    );

    if (scoredRecords.length === 0) return null;

    const total = scoredRecords.reduce(
      (sum, record) => sum + (record.bestQuizScore ?? 0),
      0,
    );
              ...
              {filterOptions.map((filter) => {
                ...
                return (
                  <Pressable
                    ...
                    style={[
                      styles.pill,
                      selected ? styles.filterPillActive : null,
                    ]}
                  ...
                  >
                    <Text
                      style={[
                        styles.pillText,
                        selected ? styles.filterPillTextActive : null,
                      ]}
                    >
                      {filter.label}
                    </Text>
